...
> **AWSとの比較:** これを ECS でやるには CodeDeploy のブルー/グリーン+ALB の加重ターゲットグループの設定が必要でした(検証用リスナーは任意ですが、リリース前検証をしたいなら実質必要になります)。Cloud Run では **Service に元から組み込まれた機能**です。
...
![デプロイとリリースの分離](imgs/deploy-release-separation.svg)
