...
> - **完成後の扱い:** `07_scaling/imgs/concurrency-and-instances.png` として保存し、**見出しの直下**に `![concurrency とインスタンス数の関係](imgs/concurrency-and-instances.png)` として差し込む(見出し → 画像 → 本文の順。キャプション文は付けない)
...
> - **完成後の扱い:** `07_scaling/imgs/scale-in-timeline.png` として保存し、**見出しの直下**に `![負荷をかけてから0台に戻るまでのタイムライン](imgs/scale-in-timeline.png)` として差し込む(見出し → 画像 → 本文の順。キャプション文は付けない)
