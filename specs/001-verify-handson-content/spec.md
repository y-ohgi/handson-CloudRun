...
- **FR-013**: 教材全体の章立て・時間配分は現行の構成(Dockerfile理解→イメージ→GAR→デプロイ→…→source deployを締めに置く、`SUMMARY.md`記載の章順・番号)を維持し、再編しない。修正は各章の記述内容(事実関係)に限定する。ただしFR-013aの条件を満たす提示方法の変更はこの限定の例外とする。
- **FR-013a**: FR-013の「事実関係に限定する」という限定の例外として、**事実の修正によって既存の提示方法(比較表の軸・表頭・○×の項目立て)が読者に誤った含意を与えることになった場合に限り**、当該の表・段落の提示方法の変更を認める。適用条件は次の3つすべてを満たすこととし、いずれかを欠く提案はFR-013に従い引き続き不採用とする。(1) 同一の章の内部で完結し、章立て・章順・時間配分・`SUMMARY.md`を変更しない。既存の節(`##` / `###`)の削除・改名・並べ替えを行わない。新しい節の追加は認めるが、追加した見出しを`fact-check-log.md`に列挙する、(2) 変更の起点となる事実の修正が`fact-check/`配下のフラグメントに一次情報の出典付きで記録されている、(3) 変更前後の提示方法の差と適用理由が`fact-check-log.md`に日付・判断者付きで記録されている。
...
- **SC-008**: FR-013aを適用した章について、変更前後で`SUMMARY.md`の差分が0行であり、既存の見出し(`##` / `###`)の文言と相対順序に差分がなく、追加された見出しがすべて`fact-check-log.md`に列挙されていることを確認できる。
...
- 当初は「比較の軸を『作成時に組む部品の数』から『運用で理解が必要なプロダクト数 / 壊れたときに開く画面の数』へ移す」という提案(`fact-check-log.md` P-10 / P-18b / A-08)を、事実の誤りではなく提示方法の変更であるとしてFR-013の外に置き不採用としていた。その後、ALBのHTTPSリスナーには証明書が必須であり独自ドメインの条件がAWS / Google Cloudで同等であることが一次情報で判明し、さらにAWS側にも1コマンドで公開まで済む経路(ECS Express Mode)が存在することが確定した。この結果「部品の数」を軸にした比較表が修正後の事実と両立しなくなったため、利用者の承認を得て2026-08-25にFR-013aとしてスコープへ追加した。FR-013の「章立て・時間配分を再編しない」という本体の制約は維持する。
