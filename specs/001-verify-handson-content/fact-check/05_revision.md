...
| 6 | L28 | 2回目以降は `--allow-unauthenticated` などを繰り返す必要がなく、サービスの設定は維持される | 未確認 | https://docs.cloud.google.com/run/docs/configuring/services/cpu , https://docs.cloud.google.com/run/docs/deploying | 「設定変更は新リビジョンを作り、以降のリビジョンは明示的に変えるまでその設定を引き継ぐ」旨は文書化されているが、「デプロイ時にフラグを省略すると既存値が保持される」および「allUsers への Invoker 付与(IAMポリシー)が再デプロイで維持される」を明言する一次情報は確認できず。実機確認項目として残し、本文は変更せず |
...
| 8 | L30 | 新リビジョン(00002)に切り替わる | 正しい | https://docs.cloud.google.com/sdk/gcloud/reference/run/deploy | `--no-traffic` を付けない限り、新リビジョンは LATEST としてトラフィックを受ける(`--no-traffic` は「デプロイするリビジョンにトラフィックを送らない」と定義)。変更不要。ただしリビジョン名の `00002-xxx` 形式自体は #12 のとおり未確認 |
...
| 12 | L30, L54, L59 | リビジョン名は `handson-app-00001-xxx` / `handson-app-00002-xxx` 形式 | 未確認 | https://docs.cloud.google.com/run/docs/managing/revisions | 公式は「サフィックスは自動割り当て(`--revision-suffix` で指定可)」とのみ記載し、`SERVICE-00001-xxx` 形式は明文化されていない。本文は「自分のリビジョン名に置き換え」と案内しているため実害は小さく、変更せず |
...
| 18 | L81 | ロールバックは「再デプロイ不要、数秒で完了」 | 未確認 | https://docs.cloud.google.com/run/docs/rollouts-rollbacks-traffic-migration | 「再デプロイ不要」は #13 のとおり正しいが、「数秒」という所要時間の一次情報はなく、公式は「瞬時ではない」とだけ述べる。#15 の修正で本文の期待値が整合したため、この表現は維持 |
