...
**2026-08-25 追記**: App Runner の新規顧客提供終了に伴う ECS Express Mode の検証は、`fact-check/aws-ecs-express-mode.md` へ分離した。本フラグメントのスコープは冒頭に記したとおり「他章に散らばる AWS 言及」であり、今回の対象は `01_aws_and_googlecloud` のサービス対応表と `03_cloudrun` の比較表という**表そのもの**であるため、ここへ加えるとスコープ宣言と食い違う。**本フラグメントの13件の判定に変更はない。**
