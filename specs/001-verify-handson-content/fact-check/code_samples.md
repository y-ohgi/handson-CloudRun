...
再現性(Node.js バージョン指定、依存の固定、Dockerfile、type stripping 適合)の検証結果です。**結論として `code/` への変更は不要でした**。検証はドキュメント突き合わせと静的確認のみで、`npm install` / `docker build` / `gcloud` は実行していません。
...
| 13 | code/websocket/src/index.ts:13 | `new Set<WSContext>()` のような呼び出し式の型引数が type stripping で扱える | 未確認(実測では通過) | https://nodejs.org/api/typescript.html | Node.js ドキュメントは呼び出し式の型引数について明記していない。`node --check code/websocket/src/index.ts` が終了コード0で通過することを実測で確認した。変更なし |
| 14 | 実測の前提 | `node --check` による構文確認 | 未確認(バージョン差) | — | 手元の Node.js は **v26.4.0** であり、教材が対象とする v24 系では確認していない。type stripping の消去可能構文の制約は v26 でも同一(v26.0.0 で `--experimental-transform-types` が削除され安定化)だが、v24 実機での確認は未実施 |
