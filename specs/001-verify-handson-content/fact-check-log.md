...
- **2026-09-01 追記(リソースタイプ名の英語表記への統一。FR-013a の適用記録)**: 判断者はリポジトリ所有者(3章の表を手動で英語化したうえで「基本的に英語でサービスの機能を紹介する」「複数形の有無を調査して全章で統一する」と指示)。実施は Claude Code の自律実行セッション。
  ...
  **変換しなかったもの**: AWS 側の「サービス」(ECS のサービス等)、一般名詞の「サービス」、Google Cloud コンソールの日本語メニュー名(「ジョブ」タブ等)、Cloud Scheduler のジョブ、AWS Batch の配列ジョブ。**オートスケールで増減するコンテナインスタンスは日本語のまま**とした。リソースタイプの `Instances` と衝突するためで、紛れる箇所は「コンテナインスタンス」と補った(`03_cloudrun/README.md`、`10_advanced/websocket.md`)。概念語(リビジョン / トラフィック分割 / タグ / 同時実行数 / コールドスタート / サイドカー)は今回のスコープ外として日本語のまま。

  **FR-013a の条件との差異**: 条件(1)は既存の節の改名を禁じているが、`10_advanced/jobs.md` の見出しを2件改名した(`## サービスとの違い` → `## Service との違い`、`## 1. ジョブを作って実行する` → `## 1. Job を作って実行する`)。用語統一が依頼の主旨であり、見出しだけカタカナで残すと統一が成立しないため。`SUMMARY.md` は章タイトル(`10-3. Cloud Run Jobs`)のみを参照し、当該H1は変更していないため目次の差分は0行。リポジトリ内に当該見出しへのアンカーリンクは存在しない。3章の見出し改名(2026-08-30 の追記)と同様、**SC-008 を字義どおりには満たさない**。この扱いは bd `handson-CloudRun-5im` で決定待ち。
