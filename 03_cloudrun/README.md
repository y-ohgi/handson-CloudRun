...
![同じコンテナイメージから分岐する3つの実行モデル](imgs/three-execution-models.svg)

Cloud Run には現在、ワークロードの形に合わせた**3つの実行モデル**があります。  
今日のハンズオンの主役はサービスですが、この3分類を頭に入れておくと「Cloud Run は HTTP リクエストを受けるだけのもの」という思い込みをせずに済みます。
...
> **[要作図] 図2: 3つの実行モデル**
>
> - **目的:** 「Cloud Run は HTTP リクエストを受けるサービスだけ」という思い込みを壊す。**同じコンテナイメージが中央にあり、そこから3方向に分岐する**という構図にすることが要点
> - **中央:** 「同じコンテナイメージ」の箱を1つ置く
> - **3方向の分岐:** それぞれ「何が起動のきっかけか」→「どう終わるか」を1組で描く
>   - サービス: HTTPリクエスト / イベント → 常に待ち受け(URLを持つ)
>   - ジョブ: 実行命令(手動 / スケジュール) → プロセスが exit したら完了
>   - ワーカープール: キューを自分で pull → 常駐しつづける(URLを持たない)
> - **補足:** ワーカープールに「URLなし」を明示すると、サービスとの違いが伝わる。AWS 対応(ECS on Fargate + ALB / ECS RunTask / SQSをポーリングするECS常駐ワーカー)を各分岐の脇に小さく添えてもよい
> - **完成後の扱い:** `03_cloudrun/imgs/three-execution-models.svg` として保存し、**見出しの直下**に `![同じコンテナイメージから分岐する3つの実行モデル](imgs/three-execution-models.svg)` として差し込む(見出し → 画像 → 本文の順。キャプション文は付けない)
...
- 実行モデルは「サービス・ジョブ・ワーカープール」の3つ。今日の主役はサービスで、概念は「サービス・リビジョン・トラフィック分割」の3つだけ。**AWS 側で ECS・ELB・CloudWatch・IAM・Application Auto Scaling に散っていた設定が、この3つと数個のつまみに集まっている**
