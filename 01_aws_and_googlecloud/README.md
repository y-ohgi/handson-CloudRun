...
> - **構図:** 上下2段(上=AWS、下=Cloud Run)。上段は AWS Cloud / Region / AZ / VPC / サブネットの入れ子フレームの中に ECS サービス・Fargate タスク・ALB・セキュリティグループ・Application Auto Scaling を置く。下段は「Cloud Run Service」1つと、そこから出る `*.run.app` の HTTPS URL
