...
> - **左(AWS/ECS):** 同じことをするのに**自分で用意する必要があったもの**——awslogs ログドライバの設定、ロググループの作成、タスク実行ロールへの `logs:CreateLogStream` / `logs:PutLogEvents` 権限——を箱で並べる
...
> - **完成後の扱い:** `08_observability/imgs/logs-and-metrics-flow.png` として保存し、**見出しの直下**に `![ログとメトリクスが勝手に集まる経路](imgs/logs-and-metrics-flow.png)` として差し込む(見出し → 画像 → 本文の順。キャプション文は付けない)
