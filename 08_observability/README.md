...
> - **右(Cloud Run):** アプリの stdout / stderr → Cloud Logging → Logs Explorer。並行して Cloud Run 自身 → Cloud Monitoring → 指標タブ。**参加者が設定した箱がゼロ**であることを強調する
> - **左(AWS/ECS):** 同じことをするのに必要だったもの——awslogs ログドライバの設定、ロググループの作成、タスク実行ロールへの `logs:CreateLogStream` / `logs:PutLogEvents` 権限——を箱で並べる
> - **要点:** 左右で「自分で用意した箱の数」を比べられる形にする。図1と同じ構図を使うと、教材を通した一貫したメッセージになる
